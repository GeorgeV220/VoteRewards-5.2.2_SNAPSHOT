...
public enum SoundUtil {
    ...
    private final static Map<String, Sound> cacheSound = Maps.newHashMap();

    public static Optional<Sound> getSound(String input) {
        if (input == null) {
            return Optional.empty();
        }
        Sound soundCache = cacheSound.get(input);
        if (soundCache != null) {
            return Optional.of(soundCache);
        }
        for (SoundUtil sound : values()) {
            for (String name : sound.versionDependentNames) {
                if (name.equalsIgnoreCase(input)) {
                    cacheSound.put(input, sound.getSound());
                    return Optional.of(sound.getSound());
                }
            }
        }
